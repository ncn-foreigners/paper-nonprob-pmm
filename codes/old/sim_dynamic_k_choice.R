...
res <- foreach(k=1:sims, .combine = rbind,
  ...
  pmm1 <- NULL
  while (REP) {
    se_prev <- if (is.null(pmm1)) Inf else pmm1$output$SE
    pmm1 <- nonprob(
      outcome = y1 ~ x1 + x2,
      data = population[flag_bd1 == 1, , drop = FALSE],
      svydesign = sample_prob,
  ...
  pmm1.1 <- NULL
  while (REP) {
    se_prev <- if (is.null(pmm1.1)) Inf else pmm1.1$output$SE
    pmm1.1 <- nonprob(
      outcome = y1 ~ x1 + x2,
      data = population[flag_bd1 == 1, , drop = FALSE],
      svydesign = sample_prob,
  ...
  pmm2 <- NULL
  while (REP) {
    se_prev <- if (is.null(pmm2)) Inf else pmm2$output$SE
    pmm2 <- nonprob(
      outcome = y2 ~ x1 + x2,
      data = population[flag_bd1 == 1, , drop = FALSE],
      svydesign = sample_prob,
  ...
  pmm2.1 <- NULL
  while (REP) {
    se_prev <- if (is.null(pmm2.1)) Inf else pmm2.1$output$SE
    pmm2.1 <- nonprob(
      outcome = y2 ~ x1 + x2,
      data = population[flag_bd1 == 1, , drop = FALSE],
      svydesign = sample_prob,
...
df <- data.frame(
  ...
  row.names = c(
    "sample 1 - yhat - y match - dynamic k",
    "sample 1 - yhat - yhat match - dynamic k",
    paste0("sample 1 - yhat - y match - stable k = ", KK2),
    paste0("sample 1 - yhat - yhat match - stable k = ", KK2),
    "sample 1 - glm",
    "sample 2 - yhat - y match - dynamic k",
    "sample 2 - yhat - yhat match - dynamic k",
    paste0("sample 2 - yhat - y match - stable k = ", KK2),
    paste0("sample 2 - yhat - yhat match - stable k = ", KK2),
    "sample 2 - yhat - glm"
  )
)
...
df |>
  group_by(y_name, est_name) |>
  group_modify(.f = function(x, y) {
    xx <- binom.test(c(sum(x$coverage), sum(1 - x$coverage)), p = .95, n = 500)
    res <- data.frame(
      xx$conf.int[1],
      xx$conf.int[2],
      xx$estimate,
