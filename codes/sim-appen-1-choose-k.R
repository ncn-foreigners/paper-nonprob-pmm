...
res <- foreach(k=1:sims, .combine = rbind,
               ...
               .options.snow = opts) %dopar% {
                 ...
                 data.frame(
                   ...
                   glm_ci = c(glm1$confidence_interval[, 1] < mean(population$y1) & mean(population$y1) < glm1$confidence_interval[, 2],
                              glm2$confidence_interval[, 1] < mean(population$y2) & mean(population$y2) < glm2$confidence_interval[, 2]),
                   
                   pmmA_dyn_ci = c(pmm1a_dyn$confidence_interval[, 1] < mean(population$y1) & mean(population$y1) < pmm1a_dyn$confidence_interval[, 2],
                                   pmm2a_dyn$confidence_interval[, 1] < mean(population$y2) & mean(population$y2) < pmm2a_dyn$confidence_interval[, 2]),
                   pmmA_fix_ci = c(pmm1a_fixed$confidence_interval[, 1] < mean(population$y1) & mean(population$y1) < pmm1a_fixed$confidence_interval[, 2],
                                   pmm2a_fixed$confidence_interval[, 1] < mean(population$y2) & mean(population$y2) < pmm2a_fixed$confidence_interval[, 2]),
                   pmmB_dyn_ci =  c(pmm1b_dyn$confidence_interval[, 1] < mean(population$y1) & mean(population$y1) < pmm1b_dyn$confidence_interval[, 2], 
                                    pmm2b_dyn$confidence_interval[, 1] < mean(population$y2) & mean(population$y2) < pmm2b_dyn$confidence_interval[, 2]),
                   pmmB_fix_ci =  c(pmm1b_fixed$confidence_interval[, 1] < mean(population$y1) & mean(population$y1) < pmm1b_fixed$confidence_interval[, 2],
                                    pmm2b_fixed$confidence_interval[, 1] < mean(population$y2) & mean(population$y2) < pmm2b_fixed$confidence_interval[, 2]))
               }
